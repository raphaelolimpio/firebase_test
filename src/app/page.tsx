...
function ScrollButton({direction, onClick}: { direction: 'left' | 'right'; onClick: () => void }) {
    return (
        <button
            onClick={onClick}
            className={`absolute top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-secondary/70 text-secondary-foreground hover:bg-secondary shadow transition-colors z-10 ${direction === 'left' ? 'left-2' : 'right-2'} group-hover:block hidden`}
        >
